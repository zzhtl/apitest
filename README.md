...
- 简洁的深色/浅色桌面界面，支持简体中文与英文。
- HTTP 请求编辑、参数、请求头、JSON/Text/XML 请求体、发送/取消和流式响应查看。
...
OAuth2、Digest、AWS SigV4 的领域模型已经预留，但当前执行器会明确返回“尚未支持”，不会静默发送错误认证信息。桌面编辑器当前聚焦 HTTP 工作流；其他协议已经在运行时层提供稳定接口，可继续接入对应编辑视图。
